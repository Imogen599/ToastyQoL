...
namespace CalNohitQoL.Core.Systems
{
    internal class SassModeSystem : ModSystem
    {
        ...
        public static void SassModeHandler(NPC boss, bool bossDead, float bossLifeRatio, float timerUnderMNL = 0)
        {
            if (bossDead)
                SassToSay = SassMode_BossDead(boss.type, timerUnderMNL);
            else if (!bossDead && timerUnderMNL > 0)
                SassToSay = SassMode_BossAliveAndUnder(boss.type, bossLifeRatio);

            if (SassToSay != null)
                CalNohitQoLUtils.DisplayText(SassToSay, Color.Orange);
            FightStatsModPlayer.currentBoss = null;
        }
        private static string SassMode_BossDead(int bossType, float timeUnderMNL)
        {
        ...
        private static string SassMode_BossAliveAndUnder(int bossType, float bossLifeRatio)
        {
            string textToReturn = null;
