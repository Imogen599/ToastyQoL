...
namespace CalNohitQoL.Core.Systems
{
    public class MNLSystem : ModSystem
    {
        ...
        public static Dictionary<int, float> DeathModeMNLs => new()
        {
        ...
        public static void DisplayMNLMessage(ref float BossAliveFrames, ref NPC Boss, bool bossDied)
        {
            if (BossAliveFrames >= 3 && Main.netMode != NetmodeID.Server)
            {
                ...
                // Some bosses have different ones in death mode, so they get set here.
                if (DeathModeMNLs.TryGetValue(Boss.type, out float deathLength) && CalamityWorld.death)
                    length = deathLength;
                ...
                if (BossAliveFrames < length)
                {
                    ...
                    //if (Boss.type == ModContent.NPCType<SupremeCalamitas>() && !DownedBossSystem.downedCalamitas)
                    //{
                    //    timerUnder += 12f;
                    //}
