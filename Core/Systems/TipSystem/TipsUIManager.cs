...
namespace CalNohitQoL.Core.Systems.TipSystem
{
    public class TipsUIManager
    {
        ...
        {
            //new BossElement("King Slime", "Slimy Monarch", "50%", TipsList.KSTipsList, new Color(50,50,255) ,ModContent.Request<Texture2D>("CalNohitQoL/Content/UI/Textures/BossIcons/kingSlime",(AssetRequestMode)1).Value, ModContent.Request<Texture2D>("CalNohitQoL/Content/UI/Textures/BossIcons/kingSlimeGlow",(AssetRequestMode)1).Value),
            //new BossElement("Desert Scourge", "Dried Worm", "N/A", TipsList.DSTipsList, new Color(200,170,0) ,ModContent.Request<Texture2D>("CalNohitQoL/Content/UI/Textures/BossIcons/desertScourge",(AssetRequestMode)1).Value, ModContent.Request<Texture2D>("CalNohitQoL/Content/UI/Textures/BossIcons/desertScourgeGlow",(AssetRequestMode)1).Value, 0.75f),
            //new BossElement("Eye of Cthulhu", "Demonic Seer", "", TipsList.EoCTipsList, new Color(50,50,255) ,ModContent.Request<Texture2D>("CalNohitQoL/Content/UI/Textures/BossIcons/eoc",(AssetRequestMode)1).Value, ModContent.Request<Texture2D>("CalNohitQoL/Content/UI/Textures/BossIcons/eoc",(AssetRequestMode)1).Value),
        };

        public BossElement CurrentBossShowing;

        public BossElement BaseBossShowing = new BossElement("base", "base", "base", new List<string>() { "base" }, Color.White, ModContent.Request<Texture2D>("CalNohitQoL/Content/UI/Textures/BossIcons/kingSlime", (AssetRequestMode)1).Value, ModContent.Request<Texture2D>("CalNohitQoL/Content/UI/Textures/BossIcons/kingSlime", (AssetRequestMode)1).Value, 1);
        ...
        public void DrawBase(SpriteBatch spriteBatch)
        {
            if (ShouldDraw)
            {
                ...
                spriteBatch.DrawString(CalNohitQoL.DraedonFont, text, closeButtonDrawPos + new Vector2(-17, -8.5f), Color.Cyan * Opacity, 0, Vector2.Zero, 0.4f, 0, 0);
                ...
                spriteBatch.DrawString(CalNohitQoL.DraedonFont, text, closeButtonDrawPos + new Vector2(-17, -8.5f), textColor * Opacity, 0, Vector2.Zero, 0.4f, 0, 0);
                #endregion
        ...
        private void DrawMainPageElements(SpriteBatch spriteBatch, Vector2 spawnPos)
        {
            #region Headers
            // Main Title Text
            spriteBatch.DrawString(CalNohitQoL.DraedonFont, "Boss Compendium", spawnPos + new Vector2(-333, -170), Color.White * Opacity, 0, Vector2.Zero, 0.8f, 0, 0);
            ...
            for (int i = 0; i < 10; i++)
            {
                if (i < BossList.Count)
                {
                    ...
                    if (MouseHitbox.Intersects(bossIconRectangle))
                    {
                        ...
                        Texture2D bloomTex = ModContent.Request<Texture2D>("CalNohitQoLAssets/ExtraTextures/Bloom", (AssetRequestMode)2).Value;
                        float opacity = 0.7f;
                        float scale2 = 0.3f + (float)Math.Sin(Main.GlobalTimeWrappedHourly) * 0.02f;
                        float rot = Main.GlobalTimeWrappedHourly * 0.5f;
                        spriteBatch.Draw(bloomTex, finalDrawPos, null, Color.Cyan * opacity, rot, new Vector2(123f, 124f), scale2, 0, 0f);
                        spriteBatch.End();
        ...
        public static void DrawCustomMouseText(SpriteBatch spriteBatch, DynamicSpriteFont font, string text, Color baseColor, bool withShadow = true)
        {
            ...
            if (font == CalNohitQoL.DraedonFont)
                ...
                scale = 1;
            spriteBatch.DrawString(font, text, position, baseColor, 0, Vector2.Zero, scale, 0, 0);
        }
    }
}
