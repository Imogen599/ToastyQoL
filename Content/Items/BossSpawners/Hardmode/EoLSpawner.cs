...
{
    public class EoLSpawner : ModItem
    {
        public int Amount = 1;
        ...
        {
            DisplayName.SetDefault("Empress Of Light Spawner");
            Tooltip.SetDefault("Instantly summons Empress Of Light\n" +
        ...
        public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
        {
            if (player.altFunctionUse == 2)
            {
                ...
                Main.NewText("Empress Of Light Spawn Count: " + Amount, TextColor);
            }
            else
            {
                for (int i = 0; i < Amount; i++)
                {
                    int idx = NPC.NewNPC(null, (int)player.Center.X - 600, (int)player.Center.Y, NPCID.HallowBoss, 1);
                }
        ...
        {
            Color color = CalNohitQoLUtils.TwoColorPulse(Color.Gold, Color.HotPink, 2);
