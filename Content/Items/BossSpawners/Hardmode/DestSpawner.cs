...
{
    public class DestSpawner : ModItem
    {
        public int Amount = 1;
        ...
        {
            DisplayName.SetDefault("Destroyer Spawner");
            Tooltip.SetDefault("Instantly summons Destroyer\n" +
        ...
        public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
        {
            if (player.altFunctionUse == 2)
            {
                ...
                if (Amount < 1)
                    Amount = 10;
                Main.NewText("Destroyer Spawn Count: " + Amount, TextColor);
            }
            else
            {
                for (int i = 0; i < Amount; i++)
                {
                    int idx = NPC.NewNPC(null, (int)player.Center.X - 600, (int)player.Center.Y, NPCID.TheDestroyer, 1);
                }
        ...
        {
            Color color = CalNohitQoLUtils.TwoColorPulse(new Color(180, 180, 204), Color.RoyalBlue, 2);
