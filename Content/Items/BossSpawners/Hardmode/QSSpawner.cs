...
{
    public class QSSpawner : ModItem
    {
        public int Amount = 1;
        ...
        {
            DisplayName.SetDefault("Queen Slime Spawner");
            Tooltip.SetDefault("Instantly summons Queen Slime\n" +
        ...
        public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
        {
            if (player.altFunctionUse == 2)
            {
                ...
                if (Amount < 1)
                    Amount = 10;
                Main.NewText("Queen Slime Spawn Count: " + Amount, TextColor);
            }
            else
            {
                for (int i = 0; i < Amount; i++)
                {
                    int idx = NPC.NewNPC(null, (int)player.Center.X - 600, (int)player.Center.Y, NPCID.QueenSlimeBoss, 1);
                }
        ...
        {
            Color color = CalNohitQoLUtils.TwoColorPulse(Color.LightPink, Color.HotPink, 2);
