...
{
    public class WoFSpawner : ModItem
    {
        public int Amount = 1;
        ...
        public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
        {
            if (player.altFunctionUse == 2)
            {
                ...
                if (Amount < 1)
                    Amount = 10;
                Main.NewText("Wall Of Flesh Spawn Count: " + Amount, TextColor);
            }
            else
            {
                for (int i = 0; i < Amount; i++)
                {
                    int idx = NPC.NewNPC(null, (int)player.Center.X - 600, (int)player.Center.Y, NPCID.WallofFlesh, 1);
                }
        ...
        {
            Color color = CalNohitQoLUtils.TwoColorPulse(Color.PaleVioletRed, new Color(194, 57, 57), 2);
