...
{
    public class DCSpawner : ModItem
    {
        public int Amount = 1;
        ...
        public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
        {
            if (player.altFunctionUse == 2)
            {
                ...
                if (Amount < 1)
                    Amount = 10;
                Main.NewText("Deerclops Spawn Count: " + Amount, TextColor);
            }
            else
            {
                for (int i = 0; i < Amount; i++)
                {
                    int idx = NPC.NewNPC(null, (int)player.Center.X - 600, (int)player.Center.Y, NPCID.Deerclops, 1);
                }
        ...
        {
            Color color = CalNohitQoLUtils.TwoColorPulse(Color.Linen, Color.DimGray, 2);
        ...
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(ItemID.DeerThing, 1);
