...
{
    public class SkPrSpawner : ModItem
    {
        public int Amount = 1;
        ...
        {
            DisplayName.SetDefault("Skeletron Prime Spawner");
            Tooltip.SetDefault("Instantly summons Skeletron Prime\n" +
        ...
        public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
        {
            if (player.altFunctionUse == 2)
            {
                ...
                if (Amount < 1)
                    Amount = 10;
                Main.NewText("Skeletron Prime Spawn Count: " + Amount, TextColor);
            }
            else
            {
                for (int i = 0; i < Amount; i++)
                {
                    int idx = NPC.NewNPC(null, (int)player.Center.X-600, (int)player.Center.Y, NPCID.SkeletronPrime, 1);
                }
        ...
        {
            Color color = CalNohitQoLUtils.TwoColorPulse(new Color(180, 180, 204), Color.OrangeRed, 2);
