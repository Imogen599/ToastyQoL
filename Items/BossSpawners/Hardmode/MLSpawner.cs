...
{
    public class MLSpawner : ModItem
    {
        public int Amount = 1;
        ...
        public override bool? UseItem(Player player)/* tModPorter Suggestion: Return null instead of false */
        {
            if (player.altFunctionUse == 2)
            {
                ...
                if (Amount < 1)
                    Amount = 10;
                Main.NewText("Moon Lord Spawn Count: " + Amount, TextColor);
            }
            else
            {
                for (int i = 0; i < Amount; i++)
                {
                    int idx = NPC.NewNPC(null, (int)player.Center.X, (int)player.Center.Y-800, NPCID.MoonLordCore, 1);
                }
        ...
        {
            Color color = CalNohitQoLUtils.TwoColorPulse(Color.Tan, Color.SpringGreen, 2);
